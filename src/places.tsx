...
function decorateText(
  ...
  if (format == 'matched') {
    ...
    return (
      <Text style={[{ color: '#c5c5c5', fontSize: 16 }, style?.suggestedText]}>
        {formatted}
      </Text>
    );
  }
  ...
  if (format == 'unmatched') {
    for (let index = 0; index < matched.length; index++) {
      ...
      formatted.splice(
        ...
        <Text
          key={randString()}
          style={[
            { fontWeight: 'normal', color: '#c5c5c5' },
            style?.suggestedText,
          ]}
        >
          {match}
    ...
    return (
      <Text
        style={[
          { color: '#121', fontSize: 16, fontWeight: 'bold' },
          style?.highlightedText,
        ]}
      >
        {formatted}
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          return (
            ...
            >
              <View
                style={[styles.suggestionItem, suggestionsStyle?.itemContainer]}
              >
                {icon ? (
        ...
        ListFooterComponent={
          data.length > 0 ? (
            <Image
              source={require('./powered_by_google.png')}
              resizeMode="contain"
              style={{ width: '32%', alignSelf: 'flex-end', marginRight: 10 }}
            />
          ) : (
            <></>
          )
